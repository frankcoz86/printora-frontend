...
const SLEEVE_PRICE_PER_METER = 3.00;
const REINFORCEMENT_SUGGESTION_AREA_THRESHOLD = 2;

const BannerPriceCalculator = ({ product, onAddToCart }) => {
  ...
  const priceCalculation = useMemo(() => {
    if (width < 50 || height < 50) return { singleBannerPrice: 0, totalPrice: 0, weldingInfo: '' };
  ...
  const handleAddToCart = useCallback(() => {
    ...
    if (hasEyelets) {
      ...
      productionNotesParts.push(`Aggiungere occhiello centrale sui lati corti (lato da ${shorterSide}cm).`);
    }
    ...
    product, onAddToCart, width, height, quantity, totalPrice,
    uploadedFile, hasReinforcement, hasEyelets, weldingInfo,
      ...
      <div className="space-y-3">
        ...
        <div className="grid grid-cols-2 gap-4 pt-2">
          <div>
            ...
            <Input id="width" type="number" value={width} onChange={(e) => setWidth(Number(e.target.value))} min="50" />
          </div>
          <div>
            <Label htmlFor="height">Altezza (cm)</Label>
            <Input id="height" type="number" value={height} onChange={(e) => setHeight(Number(e.target.value))} min="50" max={MAX_HEIGHT_CONTINUOUS} />
          </div>
        </div>
        {weldingInfo && (
      ...
      <div className="space-y-4">
                ...
                <AnimatePresence>
                  {showReinforcementSuggestion && (
                    ...
                    >
                      ...
                      <p className="text-xs font-medium">Per queste dimensioni, consigliamo il rinforzo perimetrale per una maggiore resistenza!</p>
                    </motion.div>
                  )}
                </AnimatePresence>
              </div>
        ...
        <div className="bg-slate-800/50 p-4 rounded-lg border border-slate-700">
          ...
          <div className="mt-3 space-y-3">
            <label className="flex items-center justify-between cursor-pointer">
              <span className="text-sm text-gray-200">Asola per palo/asta (+€{SLEEVE_PRICE_PER_METER.toFixed(2)}/ml)</span>
              ...
              {hasSleeve && (
                ...
                >
                  <div className="pt-3 pl-2 border-l-2 border-emerald-400 space-y-3">
                    <div>
                      <p className="text-xs text-slate-400 mb-2">Seleziona su quali lati applicare l'asola:</p>
                      <div className="grid grid-cols-2 gap-2">
                        {Object.keys(sleevePosition).map((pos) => (
                          <label key={pos} className="flex items-center space-x-2 text-sm cursor-pointer">
                            <Checkbox
                              id={`sleeve-${pos}`}
                              checked={sleevePosition[pos]}
                              onCheckedChange={() => handleSleevePositionChange(pos)}
                            />
                            <span className="capitalize">{pos === 'top' ? 'Alto' : pos === 'bottom' ? 'Basso' : pos === 'left' ? 'Sinistra' : 'Destra'}</span>
                          </label>
                        ))}
                      </div>
                    </div>
...
export default BannerPriceCalculator;

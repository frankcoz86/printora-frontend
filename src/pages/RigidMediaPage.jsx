...
import RigidMediaCard from '@/components/RigidMediaCard';

const RigidMediaPage = () => {
    const navigate = useNavigate();
    const { cartHook } = useOutletContext();

    const handleAddToCart = useCallback((product, quantity, files, total, details) => {
        if (!cartHook) return;
        cartHook.addToCart(product, quantity, files, total, details);
        toast({
                ...
                <div className="container mx-auto px-4 py-16">
                    ...
                    >
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 text-center">
                            <div className="bg-slate-800 p-8 rounded-xl border border-slate-700 hover:border-primary transition-all duration-300 transform hover:-translate-y-2">
